...
export const createDestination = async (req, res) => {
  try {
    ...
    if (!DestinationData.name || (!DestinationData.shortDescription && !DestinationData.longDescription)) {
    ...
    if (req.files?.images) {
      DestinationData.images = req.files.images.map(file => `/uploads/Destinations/${file.filename}`);
    }
    ...
    DestinationData.termsAndConditions = DestinationData.termsAndConditions || [];
...
const existingDestination = await Destination.findById(req.params.destinationId)
// console.log('existingDestination', existingDestination)
    ...
    if (req.files?.images) {
      const newImages = req.files.images.map((file) => `/uploads/Destinations/${file.filename}`);
      console.log('newImages', newImages)
      DestinationData.images = [...(existingDestination.images || []), ...newImages];
      console.log('DestinationData.images', DestinationData.images)
    }
    
if(!req.files.images && existingDestination){
    DestinationData.images= existingDestination.images
}
    if (req.files?.itineraryImages && Array.isArray(DestinationData.itineraries)) {
      let currentImageIndex = 0;
      DestinationData.itineraries = DestinationData.itineraries.map((itinerary) => {
    ...
    const destination = await Destination.findByIdAndUpdate(req.params.destinationId, DestinationData, { new: true });
...
export const getRelatedDestinations = async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: 'Error fetching related Destinations', error: error.message });
  }
}; 
...
export const updateBookingDestination = async (req, res) => {
  try {
    ...
    // Update the Destination in MongoDB
    const Destination = await Destination.findByIdAndUpdate(req.params.id, DestinationData, { new: true });
    if (!Destination) {
      console.log("Destination not found");
      return res.status(404).json({ success: false, message: "Destination not found" });
    }
