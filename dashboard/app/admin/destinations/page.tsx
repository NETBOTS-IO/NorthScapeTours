...
import { getTours, deleteTour, getDestinations, deleteDestination } from "@/lib/data-utils"
...
export default function DestinationListPage() {
  ...
  useEffect(() => {
    const fetchDestinations = async () => {
      const tours = await getDestinations()
  ...
  const handleEdit = (tourId: string) => {
    const selectedTour = destinations.find((tour) => tour.id === tourId);
    if (selectedTour) {
      useTourStore.getState().setTour(selectedTour); // Store the complete tour object
     router.push(`/admin/destinations/edit/${tourId}`);
    } else {
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    setIsDeleting(true);
    const success = await deleteDestination(deleteId);
    setIsDeleting(false);
    if (success) {
      setDestinations((prevTours) => prevTours.filter((tour) => tour.id !== deleteId));
  ...
  return (
    <div className="container">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold">Destinations Packages</h1>
        <Button className="bg-primary hover:bg-primary-dark text-white" onClick={() => router.push("/admin/destinations/add")}>
      ...
      {filteredTours.length > 0 ? (
              ...
              <TableRow key={tour.id} className="hover:bg-primary-light">
                ...
                <TableCell>
                  <div className="flex space-x-2">
                    <Link href={`/admin/destinations/${tour.id}`}>
        ...
        <p className="text-center text-gray-500">No Destination found.</p>
      )}
    </div>
  )
}
