...
const TripsListing = () => {
  ...
  let filteredTrips = tours;
  if (selectedCategory) {
    filteredTrips = filteredTrips.filter((trip) => trip.category === selectedCategory);
  }
  if (minPrice) {
  ...
  if (activities) {
    ...
    filteredTrips = filteredTrips.filter((trip) => {
      ...
      return actArr.some((a) => trip.features?.some((f) => f.toLowerCase().includes(a.toLowerCase())));
    });
  }
